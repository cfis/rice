...
TESTCASE(bracket)
{
  ...
  h[42] = 42;
  h[6] = 1;
  ASSERT_EQUAL(detail::Convert<int>::to_ruby(5), h[1].value());
  ASSERT_EQUAL(detail::Convert<int>::to_ruby(42), h[42].value());
  ASSERT_EQUAL(detail::Convert<int>::to_ruby(1), h[6].value());
}
...
TESTCASE(construct_vector_from_hash_iterators)
{
  ...
  ASSERT_EQUAL(v[0].key.value(), detail::Convert<int>::to_ruby(1));
  ASSERT_EQUAL(v[1].key.value(), detail::Convert<int>::to_ruby(6));
  ASSERT_EQUAL(v[2].key.value(), detail::Convert<int>::to_ruby(42));
  ASSERT_EQUAL(&v[0].key, &v[0].first);
  ASSERT_EQUAL(&v[1].key, &v[1].first);
  ASSERT_EQUAL(&v[2].key, &v[2].first);
  ASSERT_EQUAL(v[0].value, detail::Convert<int>::to_ruby(5));
  ASSERT_EQUAL(v[1].value, detail::Convert<int>::to_ruby(1));
  ASSERT_EQUAL(v[2].value, detail::Convert<int>::to_ruby(42));
...
TESTCASE(const_iterate)
{
  ...
  ASSERT_EQUAL(&v[1].key, &v[1].first);
  ASSERT_EQUAL(&v[2].key, &v[2].first);
  ASSERT_EQUAL(v[0].value.value(), detail::Convert<int>::to_ruby(5));
  ASSERT_EQUAL(v[1].value.value(), detail::Convert<int>::to_ruby(1));
  ASSERT_EQUAL(v[2].value.value(), detail::Convert<int>::to_ruby(42));
...
TESTCASE(iterate_and_change)
{
  ...
  {
    it->second = j;
    m[detail::Convert<int>::from_ruby(it->first)] = j;
  }
  ASSERT_EQUAL(3u, m.size());
  ASSERT_EQUAL(detail::Convert<int>::to_ruby(m[1]), h[1]);
  ASSERT_EQUAL(detail::Convert<int>::to_ruby(m[6]), h[6]);
  ASSERT_EQUAL(detail::Convert<int>::to_ruby(m[42]), h[42]);
}
